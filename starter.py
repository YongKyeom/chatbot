## Terminal 실행 명령어: streamlit run starter.py
...
def process_uploaded_file(uploaded_file):
    '''
    Data를 업로드한 후 VectorDB로 저장하는 로직
    '''
    ...
    if uploaded_file is not None:
        ...
        # Step 2) splitter => Tuning이 가능한 부분
        ...
        # Step 3) storage
        # FAISS: Facebook에서 만든 VectorDB, 굉장히 빠름
...
def generate_response(query_text, vectorstore, callback, top_k = 3):

    # Step 4) retriever 
    docs_list = vectorstore.similarity_search(query_text, k = top_k)
    docs = ""
...
def generate_summarize(raw_text, callback):
    ...
    # generator
    llm = ChatOpenAI(
        model_name = "gpt-4-1106-preview", # 참고: open ai model list 검색
        temperature = 0, 
        streaming = True, 
        callbacks=[callback]
    )
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [
        ChatMessage(
            role = "assistant", 
            content = "안녕하세요! 저는 문서에 대한 이해를 도와주는 챗봇입니다. 어떤게 궁금하신가요?"
        )
    ]
